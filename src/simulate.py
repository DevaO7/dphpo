...
def compile_tuning_results(cfg):
    ...
    if cfg.tuning.cross_validation:
        ...
        with open(os.path.join(cfg.tuning.save_path, str(similarity), f"{cfg.server.client_ratio}ur", "tuning_summary.txt"), mode='w', newline='') as file:
        ...
        for metric in evaluation_metrics:
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(os.path.join(cfg.tuning.save_path, str(similarity), f"{cfg.server.client_ratio}ur", f"{metric.lower().replace(' ', '_')}_comparison.png"))
            plt.close()
