...
class User: 
    def __init__(self, model, train_loader, test_loader, loss_fn, use_cuda, local_updates, dp, optimizer, noise_multiplier, max_grad_norm, id):
        self.use_cuda = use_cuda
        if dp:
            self.privacy_engine = PrivacyEngine()
            generator = torch.Generator(device='cuda' if use_cuda else 'cpu').manual_seed(id)
            self.model, self.optimizer, self.dp_train_loader = self.privacy_engine.make_private(
                module=self.model,
                optimizer=optimizer,
